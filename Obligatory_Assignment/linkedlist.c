...
{
    listnode_t *node = list->head;	//setter node til å være starten av listen
    while (node != NULL) {			//Så lenge det er elementer i listen
	    if (list->cmpfunc(elem, node->elem) == 0)	//Sjekk om elementet allerede er inneholdt i listen
	        return 1;
	    node = node->next;
    }
    return 0;
